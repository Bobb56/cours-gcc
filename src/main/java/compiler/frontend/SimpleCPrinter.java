...
public class SimpleCPrinter extends SimpleCBaseVisitor<String> {
	...
	@Override
	public String visitAssignStatement(SimpleCParser.AssignStatementContext ctx) {
		return visit(ctx.expr) + ";";
	}

	@Override
	public String visitDeclStatement(SimpleCParser.DeclStatementContext ctx) {
		return ctx.varType.getText() + ' ' + ctx.var.getText() + " = " + visit(ctx.expr) + ";";
	}

	@Override
	public String visitWhileStatement(SimpleCParser.WhileStatementContext ctx) {
		return "while (" + visit(ctx.expr) + ")" + visit(ctx.whileBlock);
	}

	@Override
	public String visitIfStatement(SimpleCParser.IfStatementContext ctx) {
		if (ctx.elseBlock == null) {
			return "if (" + visit(ctx.expr) + "(" + visit(ctx.ifBlock);
		}
		else {
			return "if (" + visit(ctx.expr) + ")" + visit(ctx.ifBlock) + "else" + visit(ctx.elseBlock);
		}
	}

	@Override
	public String visitForStatement(SimpleCParser.ForStatementContext ctx) {
		return "for (" + visit(ctx.expr1) + visit(ctx.expr2) + " ; " + visit(ctx.expr3) +  ")" + visit(ctx.forBlock);
	}
}
